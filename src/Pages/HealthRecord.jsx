...
export default function HealthRecord() {
    ...
    const fetchHealthRecord = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:3000/getHealthRecords/${selectedLivestock.id}`,
  ...
  const deleteHealthRecord = async (id) => {
    try {
      ...
      await axios.post(`http://localhost:3000/deleteHealthRecord/${id}`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const addRecord = async (e) => {
    ...
    try {
      ...
      await axios.post(
        "http://localhost:3000/createHealthRecord",
        addHealthRecord,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleEditSubmit = async (e) => {
    ...
    e.preventDefault();
    await axios.post(
      `http://localhost:3000/updateHealthRecord/${editHealthRecord._id}`,
      editHealthRecord,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
