...
export default function BreedingRecord() {
    ...
    const fetchBreedingRecord = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:3000/getbreedingRecords/${selectedLivestock.id}`,
          {
  ...
  const addRecord = async (e) => {
    ...
    try {
      await axios.post(
        "http://localhost:3000/createBreedingRecord",
        addBreedingRecord,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const deleteRecord = async (id) => {
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`http://localhost:3000/deleteBreedingRecord/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
