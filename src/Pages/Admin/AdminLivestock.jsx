...
export default function AdminLivestock() {
  ...
  useEffect(() => {
    const fetchLivestock = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/admin/fetchlivestock/${farmerId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await axios.post(
      `http://localhost:3000/admin/addlivestock/${farmerId}`,
      newLivestock  
    );
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    await axios.post(
      `http://localhost:3000/admin/updatelivestock/${editingLivestock._id}`,
      editingLivestock
    );
    setLivestock(
      livestock.map((item) =>
  ...
  const deleteLivestock = async (livestock) => {
    await axios.post(
      `http://localhost:3000/admin/deletels/${livestock._id}`,
