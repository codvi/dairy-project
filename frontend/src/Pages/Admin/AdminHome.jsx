...
export default function AdminHome() {
  ...
  useEffect(() => {
    const fetchFarmers = async () => {
      try {
        const res = await axios.get("http://localhost:3000/admin/fetchfarmers");
  ...
  const handleDeleteFarmer = async (farmerId) => {
    ...
    try {
      await axios.delete(`http://localhost:3000/admin/deletefarmer/${farmerId}`);
  ...
  const handleEditFarmer = async () => {
    try {
      await axios.put(`http://localhost:3000/admin/updatefarmer/${editFarmer}`, editedFarmerData);
